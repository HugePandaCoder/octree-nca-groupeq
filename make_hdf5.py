#https://github.com/choosehappy/PytorchDigitalPathology/blob/master/segmentation_epistroma_unet/make_hdf5.py
...
patch_size=320 #size of the tiles to extract and save in the database, must be >= to training size
stride_size=160 #distance to skip between patches, 1 indicated pixel wise extraction, patch_size would result in non-overlapping tiles
...
test_set_size=0.000000001 # what percentage of the dataset should be used as a held out validation/testing set
...
# +
root_path_in = "/local/scratch/clmn1/data/epi"
root_path_out = "/local/scratch/clmn1/data/epi_processed"

files=glob.glob(os.path.join(root_path_in, 'masks/*.png')) # create a list of the files, in this case we're only interested in files which have masks so we can use supervised learning
...
for phase in phases.keys(): #now for each of the phases, we'll loop through the files
    ...
    hdf5_file = tables.open_file(os.path.join(root_path_out, f"{dataname}_{phase}.pytable"), mode='w') #open the respective pytable
    ...
    for filei in phases[phase]: #now for each of the files
        ...
        for imgtype in imgtypes:
            if(imgtype=="img"): #if we're looking at an img, it must be 3 channel, but cv2 won't load it in the correct channel order, so we need to fix that
                io=cv2.cvtColor(cv2.imread(os.path.join(root_path_in, os.path.basename(fname).replace("_mask.png",".tif"))),cv2.COLOR_BGR2RGB)
