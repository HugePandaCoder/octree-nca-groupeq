...
def perform_inference_and_measure_vram(size):
    subject = "14"
    ...
    mem_allocation = torch.cuda.max_memory_allocated(model_device)
...
for mult in [20, 40, 80, 160, 170][::-1]:
    size = (mult*16, mult*16)
    num_pixel = size[0] * size[1]
    num_pixel_to_measurements[num_pixel] = []
    for _ in range(3):
        num_pixel_to_measurements[num_pixel].append(perform_inference_and_measure_vram(size))

json.dump(num_pixel_to_measurements, open("inference_pathology_unet_gpu_vram.json", "w"), indent=4)
