...
class BasicNCA3DFast(nn.Module):
    def __init__(self, channel_n, fire_rate, device, hidden_size=128, input_channels=1, init_method="standard", kernel_size=7, groups=False,
        ...
        # One Input
        self.fc0 = nn.Conv3d(channel_n*2, hidden_size, kernel_size=1)
        self.fc1 = nn.Conv3d(hidden_size, channel_n - input_channels, kernel_size=1, bias=False)
        padding = int((kernel_size-1) / 2)

        self.conv = nn.Conv3d(channel_n, channel_n, kernel_size=kernel_size, stride=1, padding=padding, padding_mode="reflect", groups=channel_n)
        ...
        if normalization == "batch":
            ...
            self.bn = torch.nn.InstanceNorm3d(hidden_size)
