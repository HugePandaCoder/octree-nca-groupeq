...
from src.utils.vitca_utils import (
    xy_meshgrid,
...
from src.utils.vitca_utils_3d import LocalizeAttention3D, clamp
    ...
    def forward(self, x, localize=None, h=None, w=None, d=None, **kwargs):
        ...
        if self.training or True:
            if localize is not None:
                ...
                k = localize(k, h, w, d)  # b h n (attn_height attn_width) d
                v = localize(v, h, w, d)  # b h n (attn_height attn_width) d

            #print(k.shape) #torch.Size([3, 4, 2400, 27, 32])
            #print(q.shape) #torch.Size([3, 4, 2400, 1, 32])
            #exit()
            ...
            #print(attn.shape) #torch.Size([3, 4, 2400, 1, 27])
            #exit()
            ...
            assert kwargs.get('mask', False) is False
            assert localize is not None
            q = rearrange(q, 'b h n d -> b h n 1 d')

            dots = torch.zeros(q.shape[0], q.shape[1], q.shape[2], 1, h, w, d, device=q.device)

            k_temp = rearrange(k, 'b h (i j k) d -> (b h) 1 i j k d', i=h, j=w, k=d)
            q_temp = rearrange(q, 'b h (i j k) 1 d -> (b h) i j k d', i=h, j=w, k=d)
            v_temp = rearrange(v, 'b h (i j k) d -> (b h) 1 i j k d', i=h, j=w, k=d)

            neighborhood_size = localize.attn_neighbourhood_size
            attn = torch.zeros(q_temp.shape[0], h, w, d, neighborhood_size[0]*neighborhood_size[1]*neighborhood_size[2], 
                                device=q.device) # (b h) h w d (attn_height attn_width)

            print(q_temp.shape[0], h, w, d, q.shape[-1])
            out = torch.zeros(q_temp.shape[0], h, w, d, self.to_out[0], device=q.device) 

            for i in range(k_temp.shape[0]):
                for out_x in range(h):
                    for out_y in range(w):
                        for out_z in range(d):
                            pixels_k = []
                            pixels_v = []
                            for in_x in range(out_x-neighborhood_size[0]//2, out_x+neighborhood_size[0]//2+1):
                                for in_y in range(out_y-neighborhood_size[1]//2, out_y+neighborhood_size[1]//2+1):
                                    for in_z in range(out_z-neighborhood_size[2]//2, out_z+neighborhood_size[2]//2+1):
                                        pixels_k.append(k_temp[i, 0, clamp(in_x, 0, h-1), clamp(in_y, 0, w-1), clamp(in_z, 0, d-1)])
                                        pixels_v.append(v_temp[i, 0, clamp(in_x, 0, h-1), clamp(in_y, 0, w-1), clamp(in_z, 0, d-1)])
                            pixels_k = torch.stack(pixels_k)
                            pixels_k = pixels_k.transpose(-1, -2)
                            pixels_v = torch.stack(pixels_v)

                            #print(pixels_k.shape)#torch.Size([32, 27])

                            dot = torch.matmul(q_temp[i,out_x,out_y,out_z], pixels_k) * self.scale
                            temp_attn = self.attend(dot)
                            attn[i, out_x, out_y, out_z] = temp_attn

                            temp_out = torch.matmul(temp_attn, pixels_v)
                            out[i, out_x, out_y, out_z] = self.to_out(temp_out)



            attn = einops.rearrange(attn, '(b h) i j k attention -> b h (i j k) 1 attention', b=q.shape[0], i=h, j=w, k=d)
            out = einops.rearrange(out, '(b h) i j k d -> b (i j k) (h d)', b=q.shape[0], i=h, j=w, k=d)
...
class ViTCA3D(torch.nn.Module):
    def __init__(self, *,
        ...
        self.localize_attn_fn = LocalizeAttention3D(localized_attn_neighbourhood, device) if localize_attn else None
        ...
        self.rearrange_cells = Rearrange('b c h w d -> b (h w d) c')
    ...
    def f(self, cells, update_rate=0.5, **kwargs):
        ...
        if self.pe_method == 'vit_handcrafted':
            ...
            raise NotImplementedError('Learned positional encoding not implemented for 3D')

        x = self.dropout(x)

        x = self.transformer(x, localize_attn_fn=self.localize_attn_fn, h=cells.shape[-3], w=cells.shape[-2], d=cells.shape[-1], **kwargs)

        # stochastic cell state update
        b, _, h, w, d = cells.shape
        update = rearrange(self.mlp_head(x), 'b (h w d) c -> b c h w d', h=h, w=w, d=d)
        if update_rate < 1.0:
            update_mask = (torch.rand(b, 1, h, w, d, device=self.device)+update_rate).floor()
    ...
    def forward(self, cells, step_n=1, update_rate=0.5, chkpt_segments=1, **kwargs):
        cells = einops.rearrange(cells, 'b h w d c -> b c h w d')
        inputs = []
        if self.octaves > 0:
            b, c, h, w = cells.shape
            octave = self.octaves
        ...
        z_star = einops.rearrange(z_star, 'b c h w d -> b h w d c')
