...
class Nii_Gz_Dataset_allPass(Nii_Gz_Dataset):
    ...
    def preprocessing(self, img, label):
        ...
        img_lowpass = self.lowpass_filter(img)
        img_highpass = self.highpass_filter(img)

        img = np.stack((img, img_lowpass, img_highpass), axis=2) #        img = np.repeat(img[:, :, np.newaxis], 3, axis=2)
        #print(img.shape)
        ...
        # Edge
        gx_m1, gy_m1 = np.gradient(label[:,:, 0])
        gx_m2, gy_m2 = np.gradient(label[:,:, 1])
        label[:, :, 2] = gy_m1 * gy_m1 + gx_m1 * gx_m1 + gy_m2 * gy_m2 + gx_m2 * gx_m2
        label[:, :, 2][label[:, :, 2] != 0.0] = 1
        #label[:,:, 2] = 0
