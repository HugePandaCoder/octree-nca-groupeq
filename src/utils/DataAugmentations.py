...
def get_augmentation_dataset(dataset_class):
    class DataAugmentationDataset(dataset_class):
        def __init__(self, *args, **kwargs):
            ...
            self.transform = []
            self.transform.append(utility_transforms.RenameTransform('label', 'target', True))
            self.transform.append(utility_transforms.RenameTransform('image', 'data', True))
            #self.transform.append(spatial_transforms.SpatialTransform()) #TODO params
            self.transform.append(noise_transforms.GaussianNoiseTransform(p_per_sample=0.1))
            self.transform.append(noise_transforms.GaussianBlurTransform((0.5, 1.), different_sigma_per_channel=True, p_per_sample=0.2,
                                               p_per_channel=0.5))
            self.transform.append(color_transforms.BrightnessMultiplicativeTransform(multiplier_range=(0.75, 1.25), p_per_sample=0.15))
            #maybe additive brightness
            self.transform.append(color_transforms.ContrastAugmentationTransform(p_per_sample=0.15))
            self.transform.append(resample_transforms.SimulateLowResolutionTransform(zoom_range=(0.5, 1), per_channel=True,
                                                        p_per_channel=0.5,
                                                        order_downsample=0, order_upsample=3, p_per_sample=0.25))
            # TODO add gamma transform
            # TODO add mirror transform

            self.transform.append(utility_transforms.RenameTransform('target', 'label', True))
            self.transform.append(utility_transforms.RenameTransform('data', 'image', True))
            self.transform = abstract_transforms.Compose(self.transform)
