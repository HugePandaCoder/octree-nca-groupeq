...
class Agent_RefineResults(Agent):
    def initialize(self):
        super().initialize()
    ...
    def batch_step(self, data, loss_f):
        ...
        #print(outputs.shape)
        #print(targets.shape)
        self.optimizer[self.exp.get_from_config('train_model')].zero_grad()
        #targets = targets.int()
        loss = 0
        loss_ret = {}
        ...
        if loss != 0:
            loss.backward()
            self.optimizer[self.exp.get_from_config('train_model')].step()
            self.scheduler[self.exp.get_from_config('train_model')].step()
        return loss_ret
    ...
    def get_outputs(self, data, full_img=False):
        ...
        if full_img == False:
            for m in range(self.exp.get_from_config('train_model')+1):
                if m == self.exp.get_from_config('train_model'):
                    ...
                    with torch.no_grad():
                        outputs = self.model[m](inputs_loc, steps=self.getInferenceSteps(), fire_rate=self.exp.get_from_config('cell_fire_rate'))
                    inputs_loc, targets_loc = self.random_crop(inputs, targets, outputs)
        else:
            for m in range(self.exp.get_from_config('train_model')+1):
                with torch.no_grad():
                    if m == self.exp.get_from_config('train_model'):
                        ...
                        inputs_loc = inputs
                        inputs_loc[:, :, :, self.exp.get_from_config('input_channels'):] = outputs[:, :, :, self.exp.get_from_config('input_channels'):]
                        targets_loc = targets
        ...
        #print("CCCCCCCC")
        #print(outputs[..., self.input_channels:self.input_channels+self.output_channels].shape)
    ...
    def resize4d(self, img, size=(64,64), factor=4, label=False):
        if label:
            transform = tio.Resize((size[0], size[1], -1), image_interpolation='NEAREST')#((int(img.shape[1]/factor), int(img.shape[2]/factor), -1), image_interpolation='NEAREST')
        else:
            transform = tio.Resize((size[0], size[1], -1))#((int(img.shape[1]/factor), int(img.shape[2]/factor), -1))
