...
class Agent_RefineResults_Upsample(Agent):
    def initialize(self):
        super().initialize()
    ...
    def get_outputs(self, data, full_img=False):
            ...
            with torch.no_grad():
                for m in range(self.exp.get_from_config('train_model')+1):
                    if m == self.exp.get_from_config('train_model'):
                        ...
                        #print(outputs.shape)     
                        inputs_loc = torch.concat((inputs_loc[...,:3], outputs[...,3:]), 3)
                        targets_loc = targets
        ...
        #print(targets_loc.shape)

        #plt.imshow(outputs[0, : , :, 0:3].cpu().detach().numpy())  
        #plt.show()
