...
config = [{
    ...
    'name': r'M3D_NCA_Run12_rewritten',
    'device':"cuda:0",
    'unlock_CPU': True,
    # Optimizer
    'lr': 16e-4,
...
ca1 = M3DNCA(config[0]['channel_n'], config[0]['cell_fire_rate'], device, hidden_size=config[0]['hidden_size'], kernel_size=5, input_channels=config[0]['input_channels'], levels=4, scale_factor=2, steps=[20, 20, 20, 20]).to(device)
ca = ca1
