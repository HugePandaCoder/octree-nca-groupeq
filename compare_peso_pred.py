...
model_path = "/local/scratch/clmn1/octree_study_new/Experiments/pesoLarge_OctreeNCA2DSegmentation"
...
img_level = 1
size = (320 + 160, 320)
slide = openslide.open_slide(f"/local/scratch/PESO/peso_training/{patient_id}.tif")
slide = slide.read_region((int(pos_x * slide.level_downsamples[img_level]),
                                int(pos_y * slide.level_downsamples[img_level])), 
                                img_level, size)
#slide = slide.read_region((20000, 60000), 1, (1024, 1024))
slide = np.array(slide)[:,:,0:3]
slide_cpu = slide

lbl_slide = openslide.open_slide(os.path.join(f"/local/scratch/PESO/peso_training/{patient_id}_training_mask.tif"))
...
seg = state[0, :,:, model.input_channels:model.input_channels + model.output_channels].cpu().numpy()

plt.imshow(slide_cpu)
plt.imshow(seg > 0, alpha=0.5)
plt.colorbar()
plt.show()