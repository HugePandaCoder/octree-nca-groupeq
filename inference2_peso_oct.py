...
state = model.backbone_ncas[4](seed.cuda(), steps=model.inference_steps[4], fire_rate=model.fire_rate)
...
if RECORD_MEMORY:
    if CUDA:
        torch.cuda.memory._dump_snapshot("mem_snapshots/inference2_peso_oct_cuda.pickle")
    else:
        torch.cuda.memory._dump_snapshot("mem_snapshots/inference2_peso_oct.pickle")
print(torch.cuda.max_memory_allocated() /1000**2)


if EXPORT_AS_ARRAY:
    state = einops.rearrange(state, "1 H W C -> H W C")
    state = state.cpu().numpy() > 0
    state = state[:,:,3]
    np.save("/local/scratch/clmn1/octree_study_new/qualitative/inference2_peso_oct.npy", state)
