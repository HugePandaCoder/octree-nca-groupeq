...
class CAModel_scale(nn.Module):
    def __init__(self, channel_n, fire_rate, device, checkCellsAlive=False, hidden_size=128):
        ...
        #torch.nn.init.normal_(self.fc0.weight, mean=0.0, std=1.0)
    ...
    def doubleHiddenLayer(self):
        ...
        fc0_copy = self.fc0
        fc1_copy = self.fc1
        self.fc0 = nn.Linear(self.channel_n*3, self.hidden_size*2)
        self.fc1 = nn.Linear(self.hidden_size*2, self.channel_n, bias=False)
        self.hidden_size = self.hidden_size*2

        self.fc0.weight.data = torch.cat((fc0_copy.weight.data, fc0_copy.weight.data), dim=0) #.resize_(self.fc0.weight.data.size(0), self.fc0.weight.data.size(1) +1)
        self.fc0.weight.data.requires_grad_(True)
        self.fc0.bias = torch.nn.parameter.Parameter(data=torch.cat((fc0_copy.bias, fc0_copy.bias), dim=0))
        self.fc0.bias.requires_grad_(True)
        self.fc1.weight.data = torch.cat((fc1_copy.weight.data, fc1_copy.weight.data), dim=1) * 0.5
        self.fc1.weight.data.requires_grad_(True)
        print(self.fc0.bias.shape)
        print(self.fc0.weight.data.shape)
        print(self.fc1.weight.data.shape)
