...
class CAModel_partiallyDead(nn.Module):
    ...
    def update(self, x_in, fire_rate, angle):
        ...
        if self.dead_mask == None:
            self.dead_mask = torch.rand([dx.size(0),dx.size(1),dx.size(2),1])>0.1
            self.dead_mask = self.dead_mask.float().to(self.device)            

        stochastic = torch.rand([dx.size(0),dx.size(1),dx.size(2),1])>fire_rate
        stochastic = stochastic.float().to(self.device)
        dx = dx * stochastic

        dx = dx * self.dead_mask
