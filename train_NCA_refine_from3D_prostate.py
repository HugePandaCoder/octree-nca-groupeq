...
def main():
    ...
    config = [{
        ...
        'model_path': r'M:\Models\TestNCA_Prostate_refine_full12',
        'device':"cuda:0",
        'n_epoch': 200,
        # Learning rate
        'lr': 16e-4,
    ...
    ca_lvl0 = LearntPerceiveNCA(config[0]['channel_n'], config[0]['cell_fire_rate'], device, hidden_size=32).to(device)
    ca_lvl1 = LearntPerceiveNCA(config[0]['channel_n'], config[0]['cell_fire_rate'], device, hidden_size=32).to(device)
    ca = [ca_lvl0, ca_lvl1]
